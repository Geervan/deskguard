...
export async function POST() {
  try {
    ...
    const now = new Date();
    // Extend break by another 20 minutes from now
    const newAwayUntil = new Date(now.getTime() + 20 * 60 * 1000);

    // Update session: extend awayUntil and clear breakGracePeriodEnd
    ...
    return NextResponse.json({
      success: true,
      message: "Break extended by 20 minutes.",
      awayUntil: newAwayUntil
    });
  } catch (error: any) {
